...
class SegFormer(nn.Module):
    ...
    def get_params(self): #TODO: Check why norm gets a 0 weight_decay and how that affects
        param_groups = [[], [], []]
    ...
    def forward(self, data, get_sup_loss = False, gt = None, criterion = None):
        ...
        pred = self.decoder(data[0]) #single modality model
        pred = F.interpolate(pred, size=(h, w), mode='bilinear', align_corners=True)
        if not self.training: #Return the ensemble predictions for evaluation
            return pred
        else: # training
            if get_sup_loss:
...
if __name__=="__main__":
    ...
    wetr = SegFormer('mit_b1', num_classes=20, embedding_dim=256, pretrained=True).cuda()
