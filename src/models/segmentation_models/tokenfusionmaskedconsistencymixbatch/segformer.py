...
class TokenFusionMaskedConsistencyMixBatch(nn.Module):
    ...
    def forward(self, data, get_sup_loss = False, gt = None, criterion = None, mask = False, range_batches_to_mask = None):
        ...
        # mask = True
        # masking_branch = [0 for _ in range(data[0].shape[0])]
        # range_batches_to_mask = [0, data[0].shape[0]]
                                 
        x, exchange_masks = self.encoder(data, mask, masking_branch, range_batches_to_mask)
