...
class NoSSLModDrop(AbstractFramework):
    
    def __init__(self, seg_model, sup_criterion, datasetClass, args):
        super(NoSSLModDrop, self).__init__(seg_model, args)
    ...
    def train(self, wandblogger, verbose = "epoch"):
        ...
        for epoch in range(self.restart_epoch, self.nepochs):
            ...
            for idx in pbar:
                ...
                pred_sup, loss_sup = self.seg_model(data, mask = True, range_batches_to_mask = range_batches_to_mask, get_sup_loss = True, gt = gt, criterion = self.sup_criterion)
