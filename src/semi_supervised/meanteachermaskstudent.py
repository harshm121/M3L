...
class MaskStudentMaskTeacher(nn.Module):
    
    def __init__(self, student, teacher, ema_alpha, mode = 'train'):
        super(MaskStudentMaskTeacher, self).__init__()
    ...
    def forward(self, data, student = True, teacher = True, mask = False, range_batches_to_mask = None, **kwargs):
        ...
        if teacher:
            if self.mode == 'train':
                ret.append(self.teacher(data, mask = mask, range_batches_to_mask = range_batches_to_mask, **kwargs))
            elif self.mode == 'val':
...
class MeanTeacherMaskStudent(AbstractFramework):
    
    def __init__(self, seg_model, sup_criterion, pseudo_sup_criterion, datasetClass, args):
        super(MeanTeacherMaskStudent, self).__init__(seg_model, args)
    ...
    def train(self, wandblogger, verbose = "epoch"):
        ...
        for epoch in range(self.n_sup_epochs, self.n_unsup_epochs+self.n_sup_epochs):
            ...
            for idx in pbar:
                ...
                gt = minibatch['gt']
                data = [d.to(self.device) for d in data]
                len_sup_data = data[0].shape[0]
                u_minibatch = unsup_dataloader.next()
                u_data = u_minibatch['data']
                u_data = [d.to(self.device) for d in u_data]
                len_unsup_data = u_data[0].shape[0]
                all_data = [torch.cat([d_s, d_u], dim = 0) for d_s, d_u in zip(data, u_data)]
                all_data_BIG = [torch.cat([d_s, d_a], dim = 0) for d_s, d_a in zip(data, all_data)]
                all_gt_BIG = torch.cat([gt, gt], dim = 0)
                all_gt_BIG = all_gt_BIG.to(self.device)

                out_student, out_teacher = self.seg_model(all_data_BIG, student = True, teacher = True, get_sup_loss = True, gt = all_gt_BIG, mask = True, 
                                                          range_batches_to_mask = [len_sup_data, 2*len_sup_data + len_unsup_data], criterion = self.sup_criterion)
    ...
    def evaluate(self, wandblogger, verbose, test_checkpoint = None, test_checkpoint_path = None):
        if test_checkpoint_path is not None:
            ...
            if verbose == "epoch":
                rangePoints = range(0, 0, self.n_sup_epochs + self.n_unsup_epochs)
